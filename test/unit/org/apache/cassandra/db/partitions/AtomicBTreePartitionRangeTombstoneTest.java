...
public class AtomicBTreePartitionRangeTombstoneTest
{
    ...
    public void testAppendRangesPreserveSemanticsAndAccounting()
    {
        ...
        canonical.collectStats(expectedStats);
        EncodingStats.Collector actualStats = new EncodingStats.Collector();
        info.collectStats(actualStats);
        assertEquals(expectedStats.get(), actualStats.get());
    ...
    @Test
    public void testMultipleRangesUseCanonicalReconciliation()
    {
        MemtableAllocator allocator = new HeapPool.Allocator(POOL);
        AtomicBTreePartition partition = newPartition(allocator);
        final int prefix = 32;
        ...
        builder.add(range(16, 10_000L));
        builder.add(range(64, 10_001L));
        ...
        assertEquals(expected.hashCode(), partition.deletionInfo().hashCode());
        assertRange(partition.deletionInfo(), 16, 10_000L);
        assertRange(partition.deletionInfo(), 64, 10_001L);
    }
