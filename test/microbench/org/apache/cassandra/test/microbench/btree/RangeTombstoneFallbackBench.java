...
/** Prices one overlapping range update after an already-published canonical 4,096-range prefix. */
...
public class RangeTombstoneFallbackBench
{
    ...
    public void setup()
    {
        ...
        metadata = TableMetadata.builder("range_tombstone_fallback_bench", "ranges")
        ...
        int start = (RANGE_COUNT / 2) * 3;
        overlappingRange = new RowUpdateBuilder(metadata, 1, RANGE_COUNT + 1L, 0)
                           .addRangeTombstone(start, start + 2)
                           .buildUpdate();
    }

    @Benchmark
    public void mergeOverlappingRangeAfter4096Prefix(StateForOverlap state, Blackhole blackhole)
    {
        state.partition.unsafeSetHolder(state.initial);
        merge(state.partition, overlappingRange, state.writeOrder);
    ...
    {
        MemtableAllocator allocator = POOL.newAllocator("range-tombstone-fallback");
    ...
    @State(Scope.Thread)
    public static class StateForOverlap
    {
        ...
        @Setup(Level.Trial)
        public void setup(RangeTombstoneFallbackBench benchmark)
        {
