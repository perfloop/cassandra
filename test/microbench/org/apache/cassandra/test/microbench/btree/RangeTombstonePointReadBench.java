...
@State(Scope.Thread)
public class RangeTombstonePointReadBench
{
    ...
    @Param({ "early", "middle", "late" })
    String position;

    private AtomicBTreePartition partition;
    private Clustering<?> point;
    ...
    public void setupTrial()
    {
        ...
        TableMetadata metadata = TableMetadata.builder("bench", "range_tombstone_point_read")
        ...
        {
            case "early":
                point = Clustering.make(ByteBufferUtil.bytes(0));
                break;
            case "middle":
                point = Clustering.make(ByteBufferUtil.bytes(RANGE_COUNT));
                break;
            case "late":
                point = Clustering.make(ByteBufferUtil.bytes(RANGE_COUNT * 2 - 2));
                break;
            default:
    ...
    @Benchmark
    public int getRowAtTombstone()
    {
        Row row = partition.getRow(point);
        return row == null ? 0 : row.clustering().hashCode();
    }
