...
@State(Scope.Thread)
public class RangeTombstoneHighCardinalityBench
{
    ...
    public void setupTrial()
    {
        ...
        TableMetadata metadata = TableMetadata.builder("bench", "range_tombstone_high_cardinality")
        ...
        // Two ordered updates produce the same successor representation as a singleton chain without making
        // high-cardinality setup part of the timed final append or batch merge.
        partition.addAll(update(metadata, key, 0, 1), cloner, writeOp, UpdateTransaction.NO_OP);
        partition.addAll(update(metadata, key, 1, existingRangeCount - 1), cloner, writeOp, UpdateTransaction.NO_OP);
        existingHolder = partition.unsafeGetHolder();
        batch = update(metadata, key, existingRangeCount, batchRangeCount);
    }
    ...
    public int mergeOrderedRangeTombstoneBatch()
    {
        ...
        int expectedRangeCount = existingRangeCount + batchRangeCount;
